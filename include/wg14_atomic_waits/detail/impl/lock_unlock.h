/* Proposed WG14 atomic wait/notify support
   (C) 2026 Niall Douglas <http://www.nedproductions.biz/>
   File Created: Jul 2026
   Licensed under the Apache License, Version 2.0 (the "License");
   you may not use this file except in compliance with the License.
   You may obtain a copy of the License in the accompanying file
   Licence.txt or at

   http://www.apache.org/licenses/LICENSE-2.0

   Unless required by applicable law or agreed to in writing, software
   distributed under the License is distributed on an "AS IS" BASIS,
   WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
   See the License for the specific language governing permissions and
   limitations under the License.
*/

#ifndef WG14_ATOMIC_WAITS_LOCK_UNLOCK_H
#define WG14_ATOMIC_WAITS_LOCK_UNLOCK_H

#include "../../config.h"
#include <assert.h>

#ifdef __cplusplus
#define WG14_ATOMIC_WAITS_ATOMIC_PREFIX std::
#else
#define WG14_ATOMIC_WAITS_ATOMIC_PREFIX
#endif

#define LOCK(x)                                                               \
  for(;;)                                                                     \
  {                                                                           \
    if(atomic_load_explicit(&(x),                                             \
                            WG14_ATOMIC_WAITS_ATOMIC_PREFIX memory_order_relaxed)) \
    { continue; }                                                             \
    unsigned expected = 0;                                                    \
    if(atomic_compare_exchange_weak_explicit(                                \
       &(x), &expected, 1, WG14_ATOMIC_WAITS_ATOMIC_PREFIX memory_order_acq_rel, \
       WG14_ATOMIC_WAITS_ATOMIC_PREFIX memory_order_relaxed))               \
    { break; }                                                                \
  }

#ifdef NDEBUG
#define UNLOCK(x)                                                             \
  atomic_store_explicit(&(x), 0,                                              \
                        WG14_ATOMIC_WAITS_ATOMIC_PREFIX memory_order_release)
#else
#define UNLOCK(x)                                                             \
  {                                                                           \
    unsigned former = atomic_exchange_explicit(                              \
    &(x), 0, WG14_ATOMIC_WAITS_ATOMIC_PREFIX memory_order_release);          \
    assert(former == 1);                                                      \
  }
#endif

#endif
